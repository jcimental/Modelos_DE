...
int background_w_fld(
  ...
  case EDE:
    ...
    *w_fld = - dOmega_ede_over_da*a/Omega_ede/3./(1.-Omega_ede)+a_eq/3./(a+a_eq);
    break;
  case JJE:
    *w_fld = -1. + pba->w0_0*cos(pba->w0_3 + 1./a -1.)/(1. + pba->w0_1*pow((1./a -1.),pba->w0_2));
    break;
  case LINEAL:
    *w_fld = pba->w0_lin1 + pba->w0_lin2*(a-1.)/a;
    break;
  case BA:
    *w_fld = pba->w0_ba1 + pba->w0_ba2*(1.-a)/(2.*pow(a,2.)-2.*a -1.);
    break;
  }
  ...
  case EDE:
    ...
    *dw_over_da_fld = - d2Omega_ede_over_da2*a/3./(1.-Omega_ede)/Omega_ede
    ...
    break;
  case JJE:
    *dw_over_da_fld = pba->w0_0*pba->w0_1*pba->w0_2*cos(pba->w0_3 + 1./a -1.)*pow(1./a -1.,pba->w0_2 -1.)/(pow(1.+pba->w0_1*pow(1./a -1.,pba->w0_2),2)*pow(a,2))
      + pba->w0_0*sin(pba->w0_3 + 1./a -1.)/(1. + pba->w0_1*pow(1./a -1.,pba->w0_2)*pow(a,2));
    break;
  case LINEAL:
    *dw_over_da_fld = -pba->w0_lin1;
    break;
  case BA:
    *dw_over_da_fld = pba->w0_ba1*(2.*pow(a,2.)-4.*a+1)/(4.*a-2.);
    break;
  }
  ...
  case JJE:
    *integral_fld = 0.;
    break;
  case LINEAL:
    *integral_fld = 3.*log(a)*(1.-pba->w0_lin1 + pba->w0_lin2)+3.*pba->w0_lin2*(1.-a);
    break;
  case BA:
    *integral_fld = -3.*(log(a)*(1.-pba->w0_ba1)+pba->w0_ba2*(0.5*log(2.*pow(a,2.)-2.*a+1.)-log(a)));
    break;
  }
...
int background_initial_conditions(
  ...
  */
  ...
  if (pba->has_fld == _TRUE_) {
    ...
    if(pba->fluid_equation_of_state == JJE) {
    /* Simpson method for JJE case */
      double funInt (
    		     double a) {
        double w0_0 = pba->w0_0;
        double w0_1 = pba->w0_1;
        double w0_2 = pba->w0_2;
        double w0_3 = pba->w0_3;
	double w_fld_jje = -1. + pba->w0_0*cos(pba->w0_3 + 1./a -1.)/(1. + pba->w0_1*pow((1./a -1.),pba->w0_2)); 	
	return 3.*(1. + w_fld_jje)/a;
      }
      double Simpson(
	             double a) {
	double b = 1;
	...
	return (3.*h/8.)*(funInt(b)+funInt(a)+Y);
      }
      a = ppr->a_ini_over_a_today_default;
      integral_fld = Simpson(a);
    }
    else {
